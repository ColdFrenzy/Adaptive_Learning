...
def dense_model(in_shape, hidden_shape,num_outputs, name):
    ...
    hidden_layer = tf.keras.layers.Dense(hidden_shape, name="layer1", activation=tf.nn.relu)(
        inputs
    )
...
def res_net_block(in_shape, hidden_shape,num_outputs, name):
    ...
    # TODO
    inputs = tf.keras.layers.Input(shape=(in_shape,), name="observations")
    dense_1 = tf.keras.layers.Dense(hidden_shape, name="dense_1", activation=tf.nn.relu)(
        inputs
    )
    ...
    out_layer = tf.keras.layers.Dense(num_outputs, name="out", activation=None)(res_layer)
