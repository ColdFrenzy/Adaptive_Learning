...
def AI_vs_AI(multiagent_env, use_lstm=False,P1_policy= "PG",P2_policy="DQN",trainer="PG",\
             stop_iters = 10,stop_timesteps = 100,reward_diff = 10, as_test = True):
    ...
    if as_test:
        raise ValueError(
            ...
            format(reward_diff, env.reward[env.player1] - env.reward[env.player2]))
...
def on_episode_step(info):
    ...
    try:
        print("Player 1 action distribution: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player1"]["action_dist_inputs"]))
        print("Player 1 action logp: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player1"]["action_logp"]))
        print("Player 1 action prob: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player1"]["action_prob"]))
    except:
        pass
    try:
        print("Player 2 action distribution: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player2"]["action_dist_inputs"]))
        print("Player 2 action logp: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player2"]["action_logp"]))
        print("Player 2 action prob: " + str(info["episode"].__dict__["_agent_to_last_pi_info"]["player2"]["action_prob"]))
    except:
        pass
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "multiagent": {
            ...
            "policies": {
                ...
                "player2": (PGTFPolicy,obs_space,act_space, {
                    "model":{
                        "custom_model": "connect4_mask",
                        "custom_model_config" : {}
                    },
    ...
    for i in range(stop_iters):
        ...
        if i%weight_update_steps == 0 and i != 0:
            ...
            for (p2_k,p2_v), (p1_k,p1_v) in zip(trainer_obj.get_policy("player2").get_weights(True).items(),
                                   trainer_obj.get_policy("player1").get_weights(True).items()):
                P2key_P1val[p2_k] = p1_v
            
            
            # set weights
            trainer_obj.set_weights({"player2":P2key_P1val, # weights or values from "player1" with "player2" keys
                                  "player1":trainer_obj.get_policy("player1").get_weights(True) # no change
                                  })
            
            print("Weight succesfully updated")
             # To check
            for (p2_k,p2_v), (p1_k,p1_v) in zip(trainer_obj.get_policy("player1").get_weights(True).items(),
    ...
    # Reward (difference) not reached: Error if `as_test`.
    if as_test:
        print("Desired reward difference ({}) not reached! Only got to {}.".
